...
\Cref{justified-api} is a small, GDP-style API based on the \texttt{justified-containers}
...
Once we have constructed a proof of type \texttt{Proof p}, we can use the \texttt{(...)} operator to attach that proof to a value of type \texttt{a}
to produce a value of type \verb|(a ::: p)|. Note that \texttt{p} will often be a proof \emph{about} the wrapped value,
...
when needed. The author of  a library dealing  with fixed-width numeric types
may be able to create domain-specific proof tactics based around the theory of bitvectors.
...
Try it with your own libraries, today!
