...
to have a function just fail on malformed inputs. The failure mode can
  be an immediate run-time error (as in \texttt{head} from \Cref{idioms-fig}), or
...
\paragraph{Returning a value with an option type.}
  ...
  error case is not possible, as in the application \texttt{headMay} to the reverse of
...
APIs that are both \emph{safe} and \emph{ergonomic}. \pdfcomment{Don't just re-define safe and ergonomic here}By \emph{safe}, we mean that the
...
a project suitable for further experimentation \cite{this}. \pdfcomment{Figure 11 seems to be missing from the repo.}
...
The proof combinators of \Cref{predicate-logic} can be seen in action
...
subset_transitive ::
  Proof (a #$\subseteq$# b) -> Proof (b #$\subseteq$# c) -> Proof (a #$\subseteq$# c)
subset_transitive _ _ = axiom
\end{minted}
...
The use of phantom type parameters to **** is well-known

Lightweight static capabilities \cite{kiselyov2006lightweight}

There are a variety of other approaches to adding **, such as Liquid Haskell,***
