...
endpts = do
  ...
  xs <- readLn
  case head xs of
    Just x -> return (x, fromJust (head (reverse xs))
    _      -> safeEndpts
----------------------------------------------------------
...
head xs = Prelude.head (the xs) -- safe!
...
\begin{figure}
  ...
  \caption{Idioms for implementing the
    ...
    \cref{full-gdp} for details. \pdfcomment{Add a local reference to this figure.}
    \label{idioms-fig}}
\end{figure}
...
of these instances\pdfcomment{Weaken?} is a vignette of a programmer
...
\textcolor{red}{The monad instance for \texttt{Proof} turns out to be very useful, enabling proofs to be written in
